...
- **Repos** du cycle sans tour → `Repos`. Un férié au repos = simple `Repos`, non payé. Les **nuits du cycle** → `Complet`/`Incomplet`/`Absent` selon les pointages.
- Le **tour de JOUR** (dimanche, ou férié en semaine par rang) porte un **statut dédié** émis par `_planning_gardien_base` : **`Garde`** (tour de jour seul) ou **`Mixte`** (tour de jour cumulé à la nuit du cycle le même jour). Ces deux statuts ne sont **jamais comptés `Absent`** (les heures réelles sont calculées depuis les pointages, le théorique = planning du jour) ; ils s'affichent `G` / `M` dans la grille (badges `badge-garde`/`badge-mixte`, cellules `cel-garde`/`cel-mixte`).
